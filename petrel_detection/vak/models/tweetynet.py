...
class TweetyNet:
    ...
    loss = nn.loss.BoundaryAwareLoss
    optimizer = torch.optim.Adam
    metrics = {
...
def set_loss_params(beta: float, gamma: float, alpha: float = 1.0):
    """供 run_experiment.py 调用，动态修改损失函数超参。

    直接修改 TweetyNet.default_config，
    不需要改文件内容，避免正则替换出错。
    """
    TweetyNet.default_config["loss"] = {
        "alpha": alpha,
        "beta": beta,
        "gamma": gamma,
    }